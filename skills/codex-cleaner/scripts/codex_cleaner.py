...
def parse_indexes(value: str) -> list[int]:
    ...
    for piece in value.replace("，", ",").split(","):
        ...
        try:
            ...
            raise SystemExit(f"Invalid index value: {piece}") from exc
    if not indexes:
        raise SystemExit("No indexes provided.")
    return indexes
...
def choose_records(
    ...
    if indexes:
        ...
        seen: set[int] = set()
        for parsed_index in parse_indexes(indexes):
            if parsed_index < 1 or parsed_index > len(records):
                raise SystemExit(f"Index {parsed_index} is out of range. Run scan to see available indexes.")
            if parsed_index not in seen:
                selected.append(records[parsed_index - 1])
                seen.add(parsed_index)
        return selected
...
def clean(args: argparse.Namespace) -> int:
    ...
    if args.project:
        selected_projects = [resolve_path(args.project)]
    else:
        selected_records = choose_records(records, args.session_id, args.archive_file, args.index, args.indexes, args.title)
    ...
    for selected_archive in selected_archives:
        if not is_relative_to(selected_archive, archive_dir):
            print(f"Refusing archive outside archive dir: {selected_archive}", file=sys.stderr)
            return 3

    for selected_project in selected_projects:
        if not args.allow_outside_codex_root and not is_relative_to(selected_project, codex_root):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    selector.add_argument("--indexes", help="Comma-separated row numbers from the scan table, such as 2,3,5.")
